...
const TaskBar = ({ task, tasks, setTasks }) => {
  const { user } = useAuth();
  ...
  return (
    <Reorder.Item
      ...
      id={task.id}
      // style={{ boxShadow, zIndex: edit ? 10 : 2 }}
      dragListener={false}
      // layout
      dragControls={dragControls}
      className="py-3 border-b w-full flex items-center gap-2 group bg-white dark:bg-dark-500 dark:border-dark-100"
    >
      {edit ? (
...
Скасувати
        ...
        <>
          ...
          <div className="flex items-center gap-2 flex-grow w-full">
            <button
              type="button"
              className="text-xl text-black/50 dark:text-white/40"
